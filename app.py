...
import numpy as np  # Para el cálculo real de desviaciones estándar (Límites Sigma)
...
# ==========================================
# ESTILO VISUAL: BUSINESS NAVY CORREGIDO AL 100%
# ==========================================
st.markdown("""
<style>
    .stApp {
...
# ==========================================
# CONEXIÓN A LA BASE DE DATOS Y LECTURA DE HISTÓRICO
# ==========================================
db_disponible = False
...
try:
    ...
    # Extraer histórico para cálculos reales del Módulo 2 y 3
    cursor.execute("SELECT efficiency FROM metric_history ORDER BY id DESC LIMIT 30;")
    historico_eficiencia = [row[0] for row in cursor.fetchall()]
    
    st.sidebar.success("✅ Conexión con Servidor Central Activa")
    db_disponible = True
except Exception:
    st.sidebar.warning("⚠️ Operando en modo local (sin datos en tiempo real)")
    db_disponible = False

# ==========================================
# DIRECCIÓN ESTRATÉGICA Y RIESGOS
# ==========================================
st.sidebar.header("⚖ shrink_to_fit Dirección Estratégica")
...
if subsistema_activo == "Módulo 1: Flujo Operativo (Lean & Gemba)":
    st.markdown("<h2 style='color: #ffffff !important;'>⚡ Módulo de Flujo Operativo Primario</h2>", unsafe_allow_html=True)
    ...
    with c1:
        st.markdown(f'<div style="background-color: #1a1a1a; padding: 25px; border-radius: 8px; border: 1px solid #003366;"><span style="font-size: 12px; text-transform: uppercase; color: #ffffff !important;">Eficiencia de Flujo Continuo</span><p style="font-size: 32px; font-weight: bold; color: #4ade80 !important; margin: 0;">{eficiencia_real:.1f}%</p></div>', unsafe_allow_html=True)
    with c2:
        st.markdown(f'<div style="background-color: #1a1a1a; padding: 25px; border-radius: 8px; border: 1px solid #772222;"><span style="font-size: 12px; text-transform: uppercase; color: #ffffff !important;">Fuga Financiera por Variabilidad Anual</span><p style="font-size: 32px; font-weight: bold; color: #ff6b6b !important; margin: 0;">${perdida_anual:,.2f} MXN</p></div>', unsafe_allow_html=True)
    with c3:
    ...
    if st.button("💾 Registrar Datos en Histórico Central"):
        if db_disponible:
            ...
            except Exception as e: st.error(f"Falla de almacenamiento: {e}")

elif subsistema_activo == "Módulo 2: Control de Variabilidad (SPC & Alertas)":
    st.markdown("<h2 style='color: #ffffff !important;'>🛡️ Módulo de Coordinación y Control de Variabilidad</h2>", unsafe_allow_html=True)
    st.markdown("<p style='color: #ffffff !important;'>El sistema analiza las desviaciones estándar en tiempo real para eliminar desperdicios en el flujo de trabajo.</p>", unsafe_allow_html=True)
    
    # CÁLCULO DE CONTROL ESTADÍSTICO REAL CON LOS DATOS DE NEON
    if len(historico_eficiencia) >= 3:
        ...
        # Evaluar desviación del último día contra los límites de control de la empresa
        if ultima_eficiencia >= (media_historica - desviacion_sigma):
            ...
            estado, color_box, color_txt = "ACCCIÓN PREVENTIVA", "#3b3010", "#facc15"
        ...
        desc = "Se requieren al menos 3 registros en el Módulo 1 para activar el cálculo dinámico de estabilidad."
    ...
    <div style="background-color: {color_box}; padding: 25px; border-radius: 8px; border-left: 8px solid {color_txt}; margin-bottom: 25px;">
        ...
        <p style="margin:5px 0 0 0; font-size:16px; color:#ffffff !important;">{desc}</p>
        <p style="font-size:24px; font-weight:bold; color:#ffffff !important; margin:10px 0 0 0;">ESTATUS: {estado} ({ultima_eficiencia:.1f}% Eficiencia Actual)</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("<h2 style='color: #ffffff !important;'>📈 Módulo de Evaluación de Desempeño y Auditoría</h2>", unsafe_allow_html=True)
    st.markdown("<p style='color: #ffffff !important;'>Este módulo consolida los registros de estabilidad procesal para alimentar los requerimientos de la <strong>Revisión por la Dirección</strong> exigida por las normas internacionales.</p>", unsafe_allow_html=True)
