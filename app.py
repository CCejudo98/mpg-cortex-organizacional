...
# Corrección de cierre de paréntesis y balance en selectbox
...
st.sidebar.markdown("---")
st.sidebar.header("📡 Radar de Risks Externos (ISO 9001:2015)")
...
if subsistema_activo == "Módulo 1: Flujo Operativo (Lean & Gemba)":
    ...
    with col1:
        ...
        e_in_real = (horas_hombre * 100.0) + (casos_atendidos * 500.0)
    ...
    # Ecuación balanceada limpiamente
    eficiencia_real = max(0.0, ((e_in_real - i_destroyed) / e_in_real) * 100.0) if e_in_real > 0.0 else 0.0
    ...
    if len(historico_eficiencia) >= 3:
        ...
        if ultima_eficiencia >= (media_historica - desviacion_sigma):
            estado, color_box, color_txt = "CONFORME", "#142e1b", "#34d399"
            ...
            estado, color_box, color_txt = "ACCCIÓN PREVENTIVA", "#4d2e0b", "#fbbf24"
            desc = "Desviación inusual detectada. El flujo presenta síntomas de fricción interna."
        else:
            estado, color_box, color_txt = "DESVIACIÓN CRÍTICA", "#451a1a", "#f87171"
            desc = "El proceso rompió límites estadísticos. Cuello de botella severo o retrabajo masivo."
    else:
        estado, color_box, color_txt = "FALTA DE DATOS", "#1c1d26", "#ffffff"
    ...
    st.markdown(f"""
    <div style="background-color: {color_box}; padding: 25px; border-radius: 8px; border-left: 8px solid {color_txt}; margin-bottom: 25px; border-top: 1px solid #2d2e3a; border-right: 1px solid #2d2e3a; border-bottom: 1px solid #2d2e3a;">
    ...
    fricciones_comunicacion = st.slider("Fallas de alineación interna reportadas:", 0, 20, 12)
    
    # Inyección limpia del diccionario estandarizado
